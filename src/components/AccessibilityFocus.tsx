        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {focusAreas.map((area, index) => (
              <div 
                key={index}
                className="group bg-card card-gradient rounded-2xl p-6 shadow-card border border-border/50 transition-smooth hover:shadow-brand hover:-translate-y-2"
              >
                <div className="space-y-6">
                  {/* Icon with gradient background */}
                  <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${area.gradient} p-3 shadow-brand`}>
                    <img 
                      src={area.icon} 
                      alt={`${area.title} icon`}
